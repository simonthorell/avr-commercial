...
void HD44780::WriteText(char *text) {
    ...
    while (*text) {
        if (charCount == 16) {  // Move to the second line after 16 characters
            GoTo(0, 1);
        } else if (charCount == 32) {  // Reset if more than 32 characters (display size)
...
void HD44780::OutNibble(unsigned char nibble) {
    if (nibble & 1) LCD_DB4_PORT |= LCD_DB4; else LCD_DB4_PORT &= ~LCD_DB4;
    if (nibble & 2) LCD_DB5_PORT |= LCD_DB5; else LCD_DB5_PORT &= ~LCD_DB5;
    if (nibble & 4) LCD_DB6_PORT |= LCD_DB6; else LCD_DB6_PORT &= ~LCD_DB6;
    if (nibble & 8) LCD_DB7_PORT |= LCD_DB7; else LCD_DB7_PORT &= ~LCD_DB7;
